<?php
    ...
    include "mysqlexecuta.php"; // Executa a cláula SQL
...
if (read_config_option("auth_method") != 0) {
	...
	$sql = "select show_tree, show_preview, show_list from user_auth where id=" . $_SESSION["sess_user_id"];
	$result = db_fetch_assoc($sql);
        if ($result[0][show_tree] == on){
                $show_graph_tab = true;
	}
	if ($result[0][show_list] == on){
                $show_graph_tab = true;
        }
	if ($result[0][show_preview] == on){
                $show_graph_tab = true;
        }
}
...
<?php 
#$sql = db_fetch_cell("select show_tree from user_auth where id=" . $_SESSION["sess_user_id"]);
#$graph_perm = db_execute($sql);
		...
		<td colspan="2" valign="bottom" nowrap>
			<table width="100%" cellspacing="0" cellpadding="0">
				<tr style="background: transparent url('<?php echo $config['url_path']; ?>images/cacti_backdrop2.gif') no-repeat center right;">
					<td id="tabs" nowrap>
						&nbsp;<?php if ($show_console_tab == true) {?><a href="<?php echo $config['url_path']; ?>index.php"><img src="<?php echo $config['url_path']; ?>images/tab_console.gif" alt="Console" align="absmiddle" border="0"></a><?php }?><?php if ($show_graph_tab == true) {?><a href="<?php echo $config['url_path']; ?>graph_view.php"><img src="<?php echo $config['url_path']; ?>images/tab_graphs<?php if ((substr(basename($_SERVER["PHP_SELF"]),0,5) == "graph") || (basename($_SERVER["PHP_SELF"]) == "graph_settings.php")) { print "_down"; } print ".gif";?>" alt="Graphs" align="absmiddle" border="0"></a><?php }?><?php
