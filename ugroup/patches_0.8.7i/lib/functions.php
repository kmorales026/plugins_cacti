...
function get_graph_tree_array($return_sql = false, $force_refresh = false) {
	...
	if (!isset($_SESSION["tree_array"]) || ($force_refresh) ||
		...
		if (read_config_option("auth_method") != 0) {
			...
			$tree_sql     = api_plugin_hook_function('auth_graph_tree_sql', array("current_user" => $current_user, "return_sql" => $return_sql));

			if (is_array($tree_sql)) {
				...
				$sql = "SELECT
					...
					ON graph_tree.id=user_auth_perms.item_id
...
function get_host_array($type = "desc_w_host") {
	if (read_config_option("auth_method") != 0) {
		$current_user = db_fetch_row("select policy_hosts from user_auth where id=" . $_SESSION["sess_user_id"]);
		$host_array   = api_plugin_hook_function('auth_host_array', $type);
		...
		if (!is_array($host_array)) {
			...
			$host_list = db_fetch_assoc("SELECT DISTINCT host.id, " .
		...
		$host_list = db_fetch_assoc("SELECT id, " .
			($type == "desc_w_host" ? "CONCAT_WS('',host.description,' (',host.hostname,')')":"description") . " as name,
...
/* get_graph_template_array - returns a list of graph_templates taking permissions into account if necessary
   @returns - (array) an array containing a list of graph_templates */
function get_graph_template_array() {
	if (read_config_option("auth_method") != 0) {
		$current_user = db_fetch_row("select policy_graph_templates from user_auth where id=" . $_SESSION["sess_user_id"]);
		$grapht_array = api_plugin_hook_function('auth_templates_array', false);

		if (!is_array($grapht_array)) {
			...
			$graph_templates = db_fetch_assoc("SELECT DISTINCT graph_templates.*,
				...
				ON ((graph_templates.id=user_auth_perms.item_id 
					...
					AND user_auth_perms.type=3 
					AND user_auth_perms.user_id=" . $_SESSION["sess_user_id"] . ") OR
					(graph_local.id=user_auth_perms.item_id
		...
		$graph_templates = db_fetch_assoc("SELECT id, name FROM graph_templates ORDER BY name");
	}
