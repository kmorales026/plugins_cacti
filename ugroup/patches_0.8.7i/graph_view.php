...
case 'tree':
	if (read_config_option("auth_method") != 0 && (empty($current_user["show_tree"]) || !api_plugin_hook_function('auth_graph_tree_allowed', true))) {
	...
	break;
case 'preview':
	if (read_config_option("auth_method") != 0 && (empty($current_user["show_preview"]) || !api_plugin_hook_function('auth_graph_preview_allowed', true))) {
				...
				<tr class="noprint">
					...
					<td width="1">
						<select name="host_id" onChange="applyGraphPreviewFilterChange(document.form_graph_view)">
							...
							<?php
							$hosts = get_host_array("description");
							if (sizeof($hosts) > 0) {
					...
					<td width="1">
						<select name="graph_template_id" onChange="applyGraphPreviewFilterChange(document.form_graph_view)">
							...
							<?php
							$graph_templates = get_graph_template_array();
	...
	if (!isset($graph_auth_array['total_rows'])) {
		...
		$total_rows = $graph_auth_array['total_rows'];
	...
	break;
case 'list':
	if (read_config_option("auth_method") != 0 && (empty($current_user["show_list"]) || !api_plugin_hook_function('auth_graph_list_allowed', true))) {
