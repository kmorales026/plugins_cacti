...
function grow_graph_tree($tree_id, $start_branch, $user_id, $options) {
	...
	$options = array('tree_id' => $tree_id, 'search_key' => $search_key, 'user_id' => $user_id);
	$tree_hierarchy = api_plugin_hook_function('auth_tree_hierarchy_array', $options);

    if (!isset($tree_hierarchy['hierarchy'])) {
		/* graph permissions */
		if (read_config_option("auth_method") != 0) {
			/* get policy information for the sql where clause */
			$sql_where = get_graph_permissions_sql($current_user["policy_graphs"], $current_user["policy_hosts"], $current_user["policy_graph_templates"]);
			$sql_where = (empty($sql_where) ? "" : "AND (" . $sql_where . " OR graph_tree_items.local_graph_id=0)");
			$sql_join = "LEFT JOIN graph_local 
		...
		$hier_sql = "SELECT
			...
			ON (graph_tree_items.id=settings_tree.graph_tree_item_id AND settings_tree.user_id=$user_id)
			LEFT JOIN host 
        ...
        $hierarchy     = $tree_hierarchy['hierarchy'];
    }
...
function grow_right_pane_tree($tree_id, $leaf_id, $host_group_data) {
	...
	if (($leaf_type == "header") || (empty($leaf_id))) {
		...
		$graph_list = db_fetch_assoc("SELECT
			...
			FROM graph_tree_items
			INNER JOIN graph_local ON graph_tree_items.local_graph_id=graph_local.id
			INNER JOIN graph_templates_graph ON (graph_local.id=graph_templates_graph.local_graph_id)
			$sql_join
			WHERE graph_tree_items.graph_tree_id=$tree_id
		...
		if ($leaf["host_grouping_type"] == HOST_GROUPING_GRAPH_TEMPLATE) {
			...
			foreach ($graph_templates as $graph_template) {
				...
				$options = array('host_id' => $leaf["host_id"], 'graph_template_id' => $graph_template["id"]);
				$graph_auth_array = api_plugin_hook_function('auth_graphs_array', $options);

				if (!isset($graph_auth_array['total_rows'])) {
					$graphs = db_fetch_assoc("SELECT
						graph_templates_graph.title_cache,
						graph_templates_graph.local_graph_id,
						...
						$sql_join
						WHERE graph_local.graph_template_id=" . $graph_template["id"] . "
						AND graph_local.host_id=" . $leaf["host_id"] . "
						$sql_where
						ORDER BY graph_templates_graph.title_cache");
				}else{
					$graphs = $graph_auth_array['graphs'];
				}
		...
		/* data query index grouping */
			...
			foreach ($data_queries as $data_query) {
				...
				if (strlen(get_request_var_request("filter"))) {
					$sql_where .= (strlen($sql_where) ? "AND":"WHERE") .  " (title_cache LIKE '%" . get_request_var_request("filter") . "%')";
				}

				$options = array('host_id' => $leaf['host_id'], 'data_query_id' => $data_query['id']);
				if (!empty($data_query_index)) {
					$options['data_query_index'] = $data_query_index;
				}
				$graph_auth_array = api_plugin_hook_function('auth_graphs_array', $options);
				...
				if (!isset($graph_auth_array['total_rows'])) {
					...
					$graphs = db_fetch_assoc("SELECT
						...
						FROM graph_local
						INNER JOIN graph_templates_graph ON graph_local.id=graph_templates_graphs.local_graph_id
						$sql_join
						WHERE graph_local.snmp_query_id=" . $data_query["id"] . "
						AND graph_local.host_id=" . $leaf["host_id"] . "
