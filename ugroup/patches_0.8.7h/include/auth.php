...
if (read_config_option("auth_method") != 0) {
	...
	if (empty($_SESSION["sess_user_id"])) {
		...
		if ($realm_id) {
			$authorized = db_fetch_cell("SELECT user_auth_realm.realm_id 
				FROM user_auth_realm 
				WHERE user_auth_realm.user_id='" . $_SESSION["sess_user_id"] . "'
				AND user_auth_realm.realm_id='$realm_id'");

			if (!$authorized) {
				$autharray = api_plugin_hook_function('auth_realm_authorized', array('authorized' => $authorized, 'realm_id' => $realm_id));
